...
import backgroundImage from "../../utils/table.jpg";
import CircularProgress from "@mui/material/CircularProgress";

const SimilarMealsForm = () => {
  ...
  const parseResponse = (apiResponse) => {
    return apiResponse.meals.meals.map((meal) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mealData) {
        setLoading(true);
        const apiResponse = await ApiService.getSimilarMeals(mealData);
        setLoading(false);
      ...
      {!response && loading && (
        ...
        >
          <Typography className="basic-response">
            The searching of similar dishes for you is underway...
          </Typography>{" "}
...
export default SimilarMealsForm;
